...
# Folder where your frontend (index.html, JS, CSS) lives
...
def get_cached_answer(query: str):
    """Check if answer exists in memory cache using cosine similarity."""
    if not qa_cache:
        return None
...
# --------------------------- #
# Helper: detect clarifying responses
# --------------------------- #

def _is_clarifying_response(text: str) -> bool:
    """Heuristic to detect if LLM returned a clarifying question instead of an answer."""
    if not text:
        return True
    ...
    starts_with_q = bool(
        re.match(r'^(what|which|who|when|where|why|how|please|could|can|would)\b', t.lower())
    )
    short_len = len(t) < 140
...
ONLY answer based on the context below. If the answer is not present in the context,
reply exactly: "Sorry, I could not find that information."
...
""".strip()
        ...
        # 4) If model asks a question back → treat as "not found"
        ...
        # 5) Only cache if it's not a 'not found' answer
...
def process_query(task_id: str, query: str):
